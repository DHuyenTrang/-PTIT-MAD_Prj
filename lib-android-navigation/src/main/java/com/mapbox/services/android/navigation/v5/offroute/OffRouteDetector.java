...
import com.mapbox.services.android.navigation.v5.turf.TurfMisc;
...
public class OffRouteDetector extends OffRoute {
  ...
  private final RingBuffer<Double> distancesAwayFromManeuver = new RingBuffer<>(3);
  private Point lastReroutePoint;
  private OffRouteCallback callback;

  /**
   * If the upcoming step is not null, detect if the current point is within the maneuver radius.
   * <p>
   * If it is, fire {@link OffRouteCallback#onShouldIncreaseIndex()} to increase the step index in
   * the <tt>NavigationEngine</tt> and return true.
   *
   * @param options      for maneuver zone radius
   * @param callback     to increase step index
   * @param currentPoint for distance from upcoming step //     * @param distanceFromUpcomingStep
   *                     for distance from current point
  ...
  private static boolean closeToUpcomingStep(MapboxNavigationOptions options, OffRouteCallback callback,
    ...
    if (currentStepIndex + 1 < legSteps.size()) {
      double maneuverZoneRadius = options.maneuverZoneRadius();
      int step = currentStepIndex + 1;
      int count = 1;

      while (step < legSteps.size()) {
        double distanceFromUpcomingStep = userTrueDistanceFromStep(currentPoint,
                legSteps.get(step));
   ...
   * <p>
   * If the current point is farther away from the maneuver than the last point in the stack, add
   * it to the stack.
   * <p>
   * If the stack if >= 3 distances, return true to fire an off-route event as it can be
   * considered that the user is no longer going in the right direction.
   *
  ...
  private static boolean movingAwayFromManeuver(RouteProgress routeProgress,
                                                RingBuffer<Double> distancesAwayFromManeuver,
...
//        Point userPointOnStep = (Point) TurfMisc.nearestPointOnLine(currentPoint, stepPoints).geometry();
    Point userPointOnStep = currentPoint;
    ...
    try {
      ...
      boolean validOffRouteDistanceTraveled =
              hasDistances && distancesAwayFromManeuver.peekLast()
      ...
      if (distancesAwayFromManeuver.isEmpty()) {
        distancesAwayFromManeuver.push(userDistanceToManeuver);
  ...
  /**
   * Method in charge of running a series of test based on the device current location and the
   * user progress along the route.
   * <p>
   * Test #1: Distance remaining.  If the route distance remaining is 0, then return true
   * immediately.  In the route processor this will prompt the snap-to-route logic to return the
   * raw Location.  If there isn't any distance remaining, the user will always be off-route.
   * <p>
   * Test #2: Valid or invalid off-route.  An off-route check can only continue if the device has
   * received at least 1 location update (for comparison) and the user has traveled passed the
   * {@link MapboxNavigationOptions#minimumDistanceBeforeRerouting()} checked against the last
   * re-route location.
   * <p>
   * Test #3: Distance from the step. This test is checked against the max of the dynamic
   * rerouting tolerance or the accuracy based tolerance. If this test passes, this method then
   * also checks if there have been &gt;= 3 location updates moving away from the maneuver point.
   * If false, this method will return false early.
   * <p>
   * Test #4: Checks if the user is close the upcoming step.  At this point, the user is
   * considered off-route. But, if the location update is within the
   * {@link MapboxNavigationOptions#maneuverZoneRadius()} of the upcoming step, this method will
   * return false as well as send fire {@link OffRouteCallback#onShouldIncreaseIndex()} to let
   * the
   * <tt>NavigationEngine</tt> know that the step index should be increased on the next location
   * update.
   *
  ...
  public boolean isUserOffRoute(Location location, RouteProgress routeProgress, MapboxNavigationOptions options) {
    ...
    if (closeToUpcomingStep(options, callback, currentPoint,
            routeProgress.currentLegProgress().stepIndex(),
            routeProgress.currentLeg().steps())) {
      return false;
    }
  ...
  /**
   * Clears the {@link RingBuffer} used for tracking our recent distances away from the maneuver
   * that is being driven towards.
   *
   * @since 0.11.0
   */
