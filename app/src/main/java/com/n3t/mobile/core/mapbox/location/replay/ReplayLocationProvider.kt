package com.n3t.mobile.core.mapbox.location.replay

import android.animation.ValueAnimator
import android.location.Location
import com.n3t.mobile.core.mapbox.navigation.PuckAnimationEvaluator
import com.n3t.mobile.core.mapbox.utils.ifNonNull
...
class ReplayLocationProvider : LocationProvider {
    ...
    fun changePosition(
        point: Point,
        bearing: Double,
        keyPoints: List<Point> = emptyList(),
        latLngTransitionOptions: (ValueAnimator.() -> Unit)? = null,
        bearingTransitionOptions: (ValueAnimator.() -> Unit)? = null
    ) {
        val location = Location("ReplayLocationProvider").apply {
            latitude = point.latitude()
            longitude = point.longitude()
            this.bearing = bearing.toFloat()
        }
        val keyPointsLocation = keyPoints.map {
            Location("ReplayLocationProvider").apply {
